...
<!-- product cards creation php -->

<div class="l-product-card-container" id="content" style="min-height: 100vh;">
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        // Modify this query to filter by category_id
        ...
        $sql = "SELECT p.product_id, p.product_name, p.product_description, p.product_price, d.discount_percent
                        FROM product AS p
                        LEFT JOIN discount AS d ON p.product_id = d.product_id
                        WHERE p.category_id = " . $_GET['category_id']; // Modify this query to filter by category_id
    }
...
<script>
    // Attach event listeners for sorting when radio buttons are changed
    const lowToHighRadio = document.getElementById("low_to_high");
    const highToLowRadio = document.getElementById("high_to_low");

    if (lowToHighRadio && highToLowRadio) {
        lowToHighRadio.addEventListener('change', function() {
            sortByLowToHigh();
        });

        highToLowRadio.addEventListener('change', function() {
            sortByHighToLow();
        });
    }
</script>
...
<script>
    ...
    // Attach an event listener to the "NewestFirst" radio button
    ...
    // Attach an event listener to the size buttons for shuffling
...
<!-- script for discount buttons -->
<!-- JavaScript code for discount filtering using jQuery -->
<script>
    // Function to filter products by discount
    function filterProductsByDiscount(discountPercent) {
        $(".product-card").each(function() {
            const cardDiscount = parseInt($(this).find(".discount").text());
    ...
    // Event listeners for discount buttons
    $(".dis-button").click(function() {
        const discountPercent = parseInt($(this).data("discount"));
